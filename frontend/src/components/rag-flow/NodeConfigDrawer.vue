  ...
  <div class="config-panel">
    ...
    <div class="panel-body" v-if="store.selectedNode">
      ...
      <div class="form-section">
        ...
        <div class="form-content">
          <component :is="formComponent" :node="store.selectedNode" @patch="onConfigPatch" />
        </div>
      </div>
    ...
    <el-dialog v-model="resultVisible" title="RAG 工作流运行结果" width="720px" :show-close="true" append-to-body>
      <div v-if="store.runtime.lastResult" class="test-result">
        <el-alert
          v-if="store.runtime.lastResult.blocked"
          title="回答已被防幻觉门禁拦截"
          type="warning"
          :description="store.runtime.lastResult.guard_issues.join('；')"
          :closable="false"
          show-icon
        />
        <el-alert
        ...
        <h4>有效原文引用 ({{ store.runtime.lastResult.validated_citations.length }})</h4>
        <div class="evidence-list" v-if="store.runtime.lastResult.validated_citations.length">
          <div v-for="(citation, i) in store.runtime.lastResult.validated_citations" :key="i" class="evidence-item">
            <div class="evidence-head"><span class="ev-source">{{ citation.source }}</span></div>
            <div class="ev-text">{{ citation.quote }}</div>
          </div>
        </div>
...
function onConfigPatch(patch: Record<string, any>) {
  if (store.selectedNode) store.setNodeConfig(store.selectedNode.id, patch)
}
