<template>
  <div class="cfg-form">
    ...
    <div class="filter-list">
      <div v-for="(f, i) in localCfg.filters" :key="i" class="filter-item">
        ...
        <el-button text size="small" @click="localCfg.filters.splice(i, 1)">
...
const localCfg = reactive({
  ...
  filters: Array.isArray(props.node.data.config?.filters) ? [...props.node.data.config.filters] : [],
})

watch(localCfg, (value) => emit('patch', { ...value, filters: [...value.filters] }), { deep: true })

function addFilter() {
  localCfg.filters.push({ field: 'source', op: 'eq', value: '' })
}
</script>
