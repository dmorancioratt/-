...
<div class="v-top-bar">
    ...
    <div class="v-main-layout">
      ...
      <main class="v-col v-col-center">
        ...
        <section class="v-chart-panel" style="flex:1;">
          <div class="v-corner-tl"></div>
          <div class="v-corner-br"></div>
          <div class="v-chart-heading"><span>岗位能力变化趋势</span><small>VERSION DELTA TREND</small></div>
          <EChart :option="versionTrendOption" class="v-version-trend" />
        </section>
      </main>
    ...
    <footer class="v-footer-strip">
      ...
      <div class="v-strip-title"><small>QUICK SWITCH</small><b>{{ footerSwitchTitle }}</b></div>
      <div class="v-role-cards">
        <button
          v-for="(card, index) in roleCards"
          :key="`${card.jobId || 'job'}-${card.date || index}-${index}`"
          type="button"
          class="v-role-card"
  ...
  <section v-else class="evo-view compare-view">
    <aside class="domain-stack">
      <button v-for="domain in leftDomains" :key="domain.domain" type="button" class="hud-panel domain-card" :class="{ active: activeDomainName === domain.domain }" @click="activeDomainName = domain.domain">
        <span class="domain-icon">{{ domainIcon(domain.domain) }}</span><div><h3>{{ domain.domain }}</h3><b>{{ domainShare(domain) }}%</b></div><ul><li v-for="skill in domain.topSkills?.slice(0, 5) || []" :key="skill.name"><span>{{ skill.name }}</span><i class="domain-skill-bar"><i :style="{ width: skillWeight(skill, domain) + '%' }"></i></i><em>{{ skillWeight(skill, domain) }}%</em></li></ul>
      </button>
    </aside>
...
const footerSwitchTitle = computed(() => new Set(displayCards.value.map((card) => card.jobId).filter(Boolean)).size <= 1 ? '历史版本快速切换' : '其他岗位快速对比')
...
const activeJobHistory = computed(() => {
  const jobId = activeCard.value.jobId
...
const versionTrendOption = computed(() => {
  const history = activeJobHistory.value
  const labels = history.map((card, index) => card.toVersion || card.date || `迭代 ${index + 1}`)
  return {
    animationDuration: 700,
    color: ['#55efff', '#ffd66b', '#ff785f'],
    tooltip: { trigger: 'axis', backgroundColor: 'rgba(2, 14, 29, .94)', borderColor: '#55efff', textStyle: { color: '#eafcff' } },
    legend: { top: 8, right: 12, data: ['新增', '调整', '淘汰'], textStyle: { color: '#7faec2', fontSize: 9 }, itemWidth: 14, itemHeight: 7 },
    grid: { left: 36, right: 18, top: 42, bottom: 28 },
    xAxis: { type: 'category', boundaryGap: false, data: labels, axisLabel: { color: '#79aabd', fontSize: 9 }, axisLine: { lineStyle: { color: 'rgba(85, 239, 255, .22)' } }, axisTick: { show: false } },
    yAxis: { type: 'value', minInterval: 1, axisLabel: { color: '#79aabd', fontSize: 9 }, splitLine: { lineStyle: { color: 'rgba(85, 239, 255, .08)', type: 'dashed' } } },
    series: [
      { name: '新增', type: 'line', smooth: .3, symbolSize: 7, data: history.map((card) => card.added?.length || 0), lineStyle: { width: 2.5 }, areaStyle: { color: 'rgba(85, 239, 255, .08)' } },
      { name: '调整', type: 'line', smooth: .3, symbolSize: 6, data: history.map((card) => card.modified?.length || 0), lineStyle: { width: 2 } },
      { name: '淘汰', type: 'line', smooth: .3, symbolSize: 6, data: history.map((card) => card.removed?.length || 0), lineStyle: { width: 2 } }
    ]
  }
})
...
watch(displayCards, (cards) => { if (activeCardIndex.value >= cards.length) activeCardIndex.value = 0 }, { deep: false })
...
function heatColor(heat: number) { return heat >= 16 ? '#ffb52e' : heat >= 12 ? '#52ddff' : heat >= 8 ? '#a26cff' : '#0aa9b4' }
...
const hotTreeKey = computed(() => allHotSkills.value.map((item) => {
  const heat = item.heat ?? item.growth ?? item.removed ?? 0
  return `${item.name}:${heat}`
}).join('|') || 'empty')
...
.v-chart-heading { position: absolute; z-index: 3; top: 10px; left: 14px; display: flex; flex-direction: column; gap: 2px; }
.v-chart-heading span { color: #dffaff; font-size: 12px; font-weight: 800; }
.v-chart-heading small { color: #52ddff; font-size: 8px; letter-spacing: .12em; }
.v-version-trend { width: 100%; height: 100%; min-height: 210px; }
...
.v-bar-fill {
  ...
  box-shadow: 0 0 5px rgba(82, 221, 255, 0.42);
}
.v-num-row {
  display: grid;
...
.v-role-card strong.up { color: #52ddff; }
.v-role-card span.warn { color: #ffd25c; }
.v-role-card i.down { color: #ff785f; font-style: normal; }

.evo-view { --cyan: #15c5c7; --line: rgba(21, 197, 199, .14); --panel: rgba(2, 24, 30, .32); position: relative; isolation: isolate; display: grid; gap: 14px; min-height: 690px; color: #d8f2f3; background: rgba(1, 12, 14, 0.05); border-radius: 20px; backdrop-filter: blur(12px) saturate(1.12); border: 1px solid rgba(21, 197, 199, 0.10); padding: 10px; }
...
.domain-stack { gap: 11px; }.domain-card { display: grid; grid-template-columns: 34px minmax(0, 1fr); gap: 10px; min-width: 0; padding: 12px; text-align: left; cursor: pointer; }.domain-card.active { border-color: #56e6ff; box-shadow: inset 0 0 22px rgba(14, 120, 193, .12), 0 0 16px rgba(39, 187, 255, .1); transform: translateX(-2px); }.domain-icon { display: grid; place-items: center; width: 34px; height: 34px; border-radius: 8px; color: #89f2ff; font-size: 12px; font-weight: 800; background: radial-gradient(circle at 30% 30%, rgba(76, 212, 255, .12), rgba(4, 35, 75, .18) 70%); box-shadow: inset 0 0 8px rgba(99, 222, 255, .08); backdrop-filter: blur(4px); }.domain-card h3 { margin: 0 0 3px; overflow: hidden; color: #ebfbff; font-size: 13px; font-weight: 900; text-overflow: ellipsis; white-space: nowrap; }.domain-card b { color: #4ee0ff; font-size: 18px; font-weight: 900; }.domain-card ul { grid-column: 1 / -1; display: grid; gap: 7px; min-width: 0; margin: 6px 0 0; padding: 0; list-style: none; }.domain-card li { display: grid; grid-template-columns: minmax(0, 1fr) 54px 26px; align-items: center; gap: 6px; min-width: 0; color: #97c5da; font-size: 9px; }.domain-card li > span { overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }.domain-skill-bar { display: block; height: 4px; overflow: hidden; background: rgba(82, 221, 255, .10); }.domain-skill-bar > i { display: block; height: 100%; min-width: 4px; background: linear-gradient(90deg, #178fb5, #52ddff); box-shadow: 0 0 5px rgba(82, 221, 255, .48); }.domain-card li em { color: #5ed2ff; font-weight: 800; font-style: normal; text-align: right; }
...
.hotspot-tree-view {
  ...
  overflow: hidden;
  background:
    radial-gradient(ellipse at 50% 20%, rgba(10, 169, 180, .12), transparent 52%),
...
.float-panel h3 {
  ...
  letter-spacing: 1px;
  padding-bottom: 8px;
  border-bottom: 1px solid rgba(82, 221, 255, 0.10);
  display: flex;
  align-items: center;
  gap: 8px;
}
...
.float-panel h3::before {
  ...
  width: 4px;
  height: 14px;
  background: linear-gradient(180deg, #52ddff, #52ddff);
  border-radius: 2px;
}
...
.bar-fill.warm { background: linear-gradient(90deg, #ffb65c, #6ec8e8); box-shadow: 0 0 8px rgba(105, 240, 174, 0.4); }
.bar-fill.cool { background: linear-gradient(90deg, #6ec8e8, #52ddff); box-shadow: 0 0 8px rgba(82, 221, 255, 0.4); }
...
.mini-skill-btn {
  ...
  width: 100%;
  padding: 8px 10px;
  background: rgba(82, 221, 255, 0.04);
...
.mini-skill-btn:hover {
  background: rgba(10, 169, 180, 0.10);
  border-color: rgba(82, 221, 255, 0.28);
...
.mini-skill-btn i {
  ...
  height: 20px;
  border-radius: 6px;
  background: linear-gradient(135deg, rgba(82, 221, 255, 0.18), rgba(10, 169, 180, 0.16));
  color: #52ddff;
  font-size: 10px;
  font-style: normal;
  font-weight: 700;
...
.mini-skill-btn em.up { color: #52ddff; }
