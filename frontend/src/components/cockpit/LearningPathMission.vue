      ...
      <div class="path-planner">
        <header class="planner-head">
          ...
          <div class="planner-title-row">
            <div><small>目标岗位</small><h2>AI 算法工程师</h2></div>
          ...
          <div class="planner-metrics">
            <span><small>当前岗位匹配</small><strong>68%</strong></span>
          ...
          <svg class="career-route" viewBox="0 0 1000 760" preserveAspectRatio="none" aria-hidden="true">
            <defs>
              <linearGradient id="careerRouteGradient" x1="0" y1="1" x2="1" y2="0">
                ...
                <stop offset=".55" stop-color="#7c7ff4" />
        ...
        <footer class="planner-foot">
          <span><i></i> 图谱已解析 1,248 条岗位—技能关系</span>
          <strong>路径随能力证据动态更新</strong>
        </footer>
      </div>
    </div>
...
const stages: LearningStage[] = [
  ...
  },
  {
    ...
    color: '#8b7cf6', rgb: '139, 124, 246', left: '55%', top: '74%', nodeCode: 'BUILD', nodeTag: '项目跃迁', skillTags: ['项目证据', '协作'], labelSide: 'right',
    resources: [
  ...
  },
  {
    ...
    color: '#ffb52e', rgb: '255, 181, 46', left: '70%', top: '45%', nodeCode: 'CHECK', nodeTag: '岗位闸门', skillTags: ['技术面', '复测'], labelSide: 'left',
    resources: [
  ...
  },
  {
    ...
    color: '#ffd45a', rgb: '255, 212, 90', left: '85.5%', top: '21.5%', nodeCode: 'GOAL', nodeTag: '目标抵达', skillTags: ['岗位机会', '成果转化'], labelSide: 'left',
    resources: [
...
const currentStage = computed(() => stages[selectedStageIndex.value])
...
.route-particle { fill: #dffcff; filter: url(#careerRouteGlow); }.route-particle--two { fill: #ffd75d; }
.route-node {
  --stage-color: #18dcf5;
...
.phase-switcher { position: relative; display: flex; justify-content: space-between; margin: 30px 8px 0; }.phase-switcher::before { content: ""; position: absolute; top: 50%; right: 4%; left: 4%; height: 1px; background: linear-gradient(90deg, #0dd8f4, #7c6bd4 52%, #b89220); }.phase-switcher button { position: relative; z-index: 1; display: grid; place-items: center; width: 47px; height: 47px; border: 1px solid rgba(39, 205, 236, .42); border-radius: 50%; background: #07162c; color: #d6e0eb; font: inherit; font-size: 16px; cursor: pointer; transition: .25s ease; }.phase-switcher button:nth-child(3) { border-color: rgba(156, 125, 245, .55); }.phase-switcher button:nth-child(n+4) { border-color: rgba(255, 186, 25, .55); }.phase-switcher button:hover,.phase-switcher button.active { border-color: #11d8f4; background: #12d7f2; color: #041221; box-shadow: 0 0 22px rgba(18, 215, 242, .65); transform: scale(1.06); }.phase-switcher button.started::after { content: ""; position: absolute; top: -3px; right: -1px; width: 8px; height: 8px; border-radius: 50%; background: #70f2b3; box-shadow: 0 0 8px #70f2b3; }
.resource-heading { display: flex; align-items: center; justify-content: space-between; gap: 16px; margin-top: 29px; }.resource-heading h2 { margin: 0; font-size: 18px; }.start-stage { display: flex; align-items: center; gap: 8px; border: 0; border-radius: 9px; padding: 12px 16px; background: linear-gradient(135deg, #10c8ee, #0697c9); color: #03111f; font: inherit; font-size: 13px; font-weight: 800; cursor: pointer; box-shadow: 0 12px 28px rgba(2, 183, 224, .18); }.start-stage svg { width: 19px; fill: none; stroke: currentColor; stroke-width: 1.8; }.start-stage:hover { transform: translateY(-2px); box-shadow: 0 16px 32px rgba(2, 197, 235, .3); }.start-stage.active { background: linear-gradient(135deg, #73f5bc, #19c8a0); }
