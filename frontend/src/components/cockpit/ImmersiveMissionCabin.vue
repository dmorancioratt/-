<template>
  <Transition name="mission-cabin" appear>
    ...
    >
      ...
      <div class="mission-cabin__scan" aria-hidden="true"></div>
          ...
          <div class="visual-stage">
            ...
            <template v-else-if="config.visual === 'ai'">
              <div class="ai-map">
                ...
                <div class="ai-map__core"><span>AI</span><b>DECISION</b><small>87% CONFIDENCE</small></div>
                <button
                  v-for="(node, index) in config.nodes"
                  :key="node.id"
                  type="button"
      ...
      <footer class="mission-footer">
        <span><i></i> LIVE DATA · UPDATED 09:42</span>
        <span>AI REASONING TRACE AVAILABLE</span>
...
const props = defineProps<{ moduleId: MissionCabinId }>()
...
const config = computed(() => missionCabins[props.moduleId])
...
function toggleTask(id: string) {
  tasks.value = tasks.value.map(task => task.id === id ? { ...task, done: !task.done } : task)
}
...
.mission-cabin__veil,
.mission-cabin__grid,
.mission-cabin__scan { position: absolute; inset: 0; pointer-events: none; }
.mission-cabin__veil {
  background:
...
.mission-cabin__scan {
  width: 36%;
  background: linear-gradient(90deg, transparent, rgba(var(--mission-rgb), .08), transparent);
  animation: mission-scan 5.5s linear infinite;
}
...
.ai-map { position: absolute; inset: 8% 7% 21%; }
.ai-map__core { position: absolute; top: 50%; left: 50%; display: grid; place-items: center; width: 170px; aspect-ratio: 1; border: 1px solid var(--mission-accent); border-radius: 50%; background: radial-gradient(circle, rgba(var(--mission-rgb), .28), rgba(4, 12, 31, .96) 66%); transform: translate(-50%, -50%); box-shadow: 0 0 70px rgba(var(--mission-rgb), .26); }.ai-map__core span { color: #fff; font-size: 42px; font-weight: 900; }.ai-map__core b { color: var(--mission-accent); font-size: 9px; letter-spacing: .22em; }.ai-map__core small { color: #728f9d; font-size: 8px; }.ai-map__halo { position: absolute; top: 50%; left: 50%; border: 1px solid rgba(var(--mission-rgb), .24); border-radius: 50%; transform: translate(-50%,-50%); }.ai-map__halo--one { width: 310px; height: 310px; }.ai-map__halo--two { width: 430px; height: 430px; border-style: dashed; animation: mission-spin 30s linear infinite; }
...
@keyframes mission-scan { from { transform: translateX(-120%); } to { transform: translateX(380%); } }
...
@media (max-width: 760px) {
  ...
  .profile-map { grid-template-columns: 1fr; overflow: auto; }.profile-avatar { display: none; }.resource-map { grid-template-columns: 1fr; overflow-y: auto; }.calendar-map { grid-template-columns: repeat(4, 1fr); overflow-y: auto; }.calendar-map i { min-height: 70px; }.achievement-map { align-items: flex-start; overflow-x: auto; }.achievement-map button { min-width: 120px; }.mission-footer { display: none; }
}
