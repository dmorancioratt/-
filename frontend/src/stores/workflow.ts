      ...
      try {
        ...
        for (const [stageLabel, kind] of kindToStage) {
          ...
          if (node && log) {
            ...
            node.data.status = valid.includes(log.status as NodeStatus) ? log.status as NodeStatus : 'idle'
            node.data.output = log.output
          }
        }
    ...
    applyStreamEvent(payload: any) {
      if (payload.event === 'stage') {
        ...
        this.runtime.lastResult = {
          ...
          validated_citations: payload.validated_citations || [],
