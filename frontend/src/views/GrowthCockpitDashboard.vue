        ...
        <section class="gcd-panel gcd-panel--grow">
          ...
          <div class="gcd-ring">
            <svg viewBox="0 0 200 200" class="gcd-ring__svg">
              ...
              <circle cx="100" cy="100" r="84" fill="none" stroke="rgba(34,247,255,0.08)" stroke-width="10" />
              ...
              <circle cx="100" cy="100" r="60" fill="none" stroke="rgba(34,247,255,0.06)" stroke-width="1" stroke-dasharray="3 5" />
            </svg>
            ...
            <svg viewBox="0 0 260 200" class="gcd-radar__svg">
              <defs>
                <linearGradient id="radarFill" x1="0" y1="0" x2="1" y2="1">
                  <stop offset="0%" stop-color="#22f7ff" stop-opacity="0.32" />
                  <stop offset="100%" stop-color="#1f6fae" stop-opacity="0.10" />
                </linearGradient>
              </defs>
              <polygon v-for="lv in [1, 0.75, 0.5, 0.25]" :key="lv" :points="radarPts(100 * lv)" fill="none" stroke="rgba(34,247,255,0.10)" stroke-width="1" />
              <line v-for="(d, i) in radarDims" :key="'ax' + i" :x1="130" :y1="100" :x2="radarPt(d, 100).x" :y2="radarPt(d, 100).y" stroke="rgba(34,247,255,0.10)" stroke-width="1" />
              <polygon :points="radarPtsData" fill="url(#radarFill)" stroke="#22f7ff" stroke-width="2" filter="url(#ringGlow)" />
              <circle v-for="(d, i) in radarDims" :key="'pt' + i" :cx="radarPt(d, d.value).x" :cy="radarPt(d, d.value).y" r="3.5" fill="#22f7ff" />
            ...
            <svg viewBox="0 0 420 220" preserveAspectRatio="none" class="gcd-line__svg">
              <defs>
                <linearGradient id="lineFill" x1="0" y1="0" x2="0" y2="1">
                  <stop offset="0%" stop-color="#22f7ff" stop-opacity="0.26" />
                  <stop offset="100%" stop-color="#22f7ff" stop-opacity="0" />
                </linearGradient>
              </defs>
              <line v-for="g in 5" :key="g" :x1="0" :x2="420" :y1="(g - 1) * 44" :y2="(g - 1) * 44" stroke="rgba(34,247,255,0.08)" stroke-width="1" />
              <path d="M10,40 C90,58 140,30 200,96 C260,162 330,130 410,180" fill="none" stroke="#22f7ff" stroke-width="2.5" stroke-linecap="round" filter="url(#ringGlow)" />
              <path d="M10,40 C90,58 140,30 200,96 C260,162 330,130 410,180 L410,220 L10,220 Z" fill="url(#lineFill)" />
              <circle cx="10" cy="40" r="4" fill="#0b2237" stroke="#22f7ff" stroke-width="2" />
              <circle cx="410" cy="180" r="4" fill="#0b2237" stroke="#22f7ff" stroke-width="2" />
            </svg>
          </div>
...
.gcd {
  ...
  --gcd-border: rgba(34, 247, 255, 0.09);
  --gcd-cyan: #22f7ff;
  --gcd-blue: #4feaff;
  --gcd-teal: #00c9d2;
  --gcd-amber: #ffc048;
  ...
  color: var(--gcd-text);
  background:
    radial-gradient(120% 90% at 50% 0%, rgba(34, 247, 255, 0.06), transparent 55%),
...
.gcd__back { display: inline-flex; align-items: center; gap: 6px; border: 1px solid rgba(34, 247, 255, 0.32); border-radius: 10px; padding: 8px 14px; color: var(--gcd-cyan); background: rgba(0, 201, 210, 0.10); font: inherit; font-size: 12px; font-weight: 700; cursor: pointer; transition: all 0.25s; flex-shrink: 0; }
.gcd__back:hover { background: rgba(0, 201, 210, 0.20); border-color: rgba(34, 247, 255, 0.55); }
...
.gcd__title p { margin: 6px 0 0; color: var(--gcd-cyan); font-size: 11px; font-weight: 800; letter-spacing: 0.32em; text-transform: uppercase; text-shadow: 0 0 8px rgba(34, 247, 255, 0.55); }
.gcd__live { display: flex; align-items: center; gap: 16px; }
.gcd__live-badge { display: inline-flex; align-items: center; gap: 7px; border: 1px solid rgba(34, 247, 255, 0.32); border-radius: 999px; padding: 6px 12px; color: var(--gcd-cyan); font-size: 11px; font-weight: 700; background: rgba(0, 201, 210, 0.10); box-shadow: 0 0 14px rgba(34, 247, 255, 0.12); }
.gcd__live-badge i { width: 7px; height: 7px; border-radius: 50%; background: var(--gcd-cyan); box-shadow: 0 0 7px rgba(34, 247, 255, 0.8); animation: gcd-pulse 1.6s ease-in-out infinite; }
...
.gcd-panel__bar { width: 3px; height: 14px; border-radius: 2px; background: linear-gradient(180deg, var(--gcd-cyan), #1f6fae); box-shadow: 0 0 10px rgba(34, 247, 255, 0.5); }
...
.gcd-ring__center b { color: #eaffff; font-size: 46px; line-height: 1; text-shadow: 0 0 22px rgba(34, 247, 255, 0.55); }
...
.gcd-kv em { color: var(--gcd-cyan); font-size: 20px; font-weight: 800; font-style: normal; text-shadow: 0 0 10px rgba(34, 247, 255, 0.5); }
...
.gcd-span__line { height: 2px; background: linear-gradient(90deg, var(--gcd-cyan), rgba(0, 201, 210, 0.18)); box-shadow: 0 0 8px rgba(34, 247, 255, 0.5); }
...
.gcd-node__dot { width: 9px; height: 9px; border-radius: 50%; background: var(--gcd-cyan); box-shadow: 0 0 10px rgba(34, 247, 255, 0.7); flex-shrink: 0; }
...
/* 目标角色 */
.gcd-role { margin: 0; color: var(--gcd-text); font-size: 20px; font-weight: 800; text-shadow: 0 0 14px rgba(34, 247, 255, 0.35); }

/* 跃迁路线 */
.gcd-route { display: grid; gap: 10px; margin: 0; padding: 0; list-style: none; }
.gcd-route li { display: flex; align-items: center; gap: 12px; }
.gcd-route__idx { display: grid; place-items: center; width: 26px; height: 26px; border: 1px solid rgba(34, 247, 255, 0.28); border-radius: 7px; color: var(--gcd-cyan); font-size: 11px; font-weight: 700; flex-shrink: 0; }
...
.gcd-evidence__tag { border-radius: 5px; padding: 3px 8px; color: var(--gcd-cyan); background: rgba(0, 201, 210, 0.12); font-size: 11px; font-weight: 700; }
...
.gcd-score__num { color: var(--gcd-text); font-size: 40px; font-weight: 850; letter-spacing: -0.02em; text-shadow: 0 0 18px rgba(34, 247, 255, 0.4); }
...
.gcd-insight li b.up { color: #9ad8c5; }
.gcd-insight li b.warn { color: var(--gcd-amber); }
.gcd-insight li small { color: var(--gcd-muted); font-size: 11px; line-height: 1.5; }

.gcd-insight__mark { position: absolute; right: 0; bottom: -2px; color: rgba(34, 247, 255, 0.16); font-size: 40px; font-weight: 900; letter-spacing: 0.2em; line-height: 1; pointer-events: none; }
