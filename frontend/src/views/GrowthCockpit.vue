<template>
  <div class="cockpit-container" :class="{ 'panel-open': activePanel }">
    ...
    <div v-if="loading" class="data-sync-state">正在同步当前账号数据...</div>
    <button v-else-if="!activePanel" class="data-sync-button" type="button" @click="loadData">同步数据</button>
    ...
    <GrowthProfileMission
      ...
      :interview-count="interviews.length"
      :resumes="resumes"
      :matches="matches"
    ...
    <LearningPathMission
      ...
      :target-job="latestMatch?.target_job || profile.target_role || ''"
    ...
    <ImmersiveMissionCabin
      ...
      :live-tasks="learningTasks"
      :live-resources="learningResources"
      :suggestions="suggestions"
      ...
      <div class="detail-inner">
        ...
        <div v-if="activePanel === 'match'" class="detail-page page-match">
          <div class="page-header">
            <div class="header-icon" style="background: linear-gradient(135deg, #52ddff20, #0aa9b420); border-color: #52ddff60;">
              <svg viewBox="0 0 24 24" fill="none" stroke="#52ddff" stroke-width="2"><circle cx="12" cy="12" r="10"/><path d="M12 6v6l4 2"/></svg>
            </div>
            <div class="header-titles">
              <h2>岗位匹配度分析</h2>
              <p>AI算法工程师 · 字节跳动</p>
            </div>
            <div class="header-badge" style="background: linear-gradient(135deg, #52ddff, #0aa9b4); color: #050d1f;">72%</div>
          </div>
              ...
              <svg class="big-ring-svg" viewBox="0 0 240 240">
                <defs>
                  <linearGradient id="ringGrad" x1="0%" y1="0%" x2="100%" y2="100%">
                    ...
                    <stop offset="50%" stop-color="#52ddff"/>
                    <stop offset="100%" stop-color="#0aa9b4"/>
                  </linearGradient>
              ...
              <svg viewBox="0 0 500 160" class="trend-svg">
                <defs>
                  <linearGradient id="trendFill" x1="0%" y1="0%" x2="0%" y2="100%">
                    <stop offset="0%" stop-color="#52ddff" stop-opacity="0.3"/>
                ...
                <polyline :points="trendPoints" fill="none" stroke="#52ddff" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" style="filter: drop-shadow(0 0 6px #52ddff80)"/>
                <polygon :points="trendPoints + ' 480,150 40,150'" fill="url(#trendFill)"/>
                <circle v-for="(p, i) in trendData" :key="i" :cx="40 + i * 62.8" :cy="150 - p.val * 1.1" r="5" fill="#050d1f" stroke="#52ddff" stroke-width="2"/>
        ...
        <div v-if="activePanel === 'radar'" class="detail-page page-radar">
          <div class="page-header">
            <div class="header-icon" style="background: linear-gradient(135deg, #52ddff20, #52ddff20); border-color: #52ddff60;">
              <svg viewBox="0 0 24 24" fill="none" stroke="#52ddff" stroke-width="2"><polygon points="12 2 22 8.5 22 15.5 12 22 2 15.5 2 8.5"/><line x1="12" y1="2" x2="12" y2="22"/><line x1="2" y1="8.5" x2="22" y2="15.5"/><line x1="22" y1="8.5" x2="2" y2="15.5"/></svg>
            </div>
            <div class="header-titles">
              <h2>能力图谱</h2>
            ...
            <svg class="radar-big-svg" viewBox="0 0 400 400">
              <defs>
                <radialGradient id="radarFill" cx="50%" cy="50%" r="50%">
                  ...
                  <stop offset="100%" stop-color="#0aa9b4" stop-opacity="0.1"/>
                </radialGradient>
              ...
              <g transform="translate(200,200)">
                ...
                <polygon :points="radarDataPts" fill="url(#radarFill)" stroke="#52ddff" stroke-width="2" filter="url(#radarGlow)"/>
                <circle v-for="(s, i) in radarSkills" :key="'c'+i" :cx="160 * (s.val/100) * Math.cos(Math.PI*2*i/radarSkills.length - Math.PI/2)" :cy="160 * (s.val/100) * Math.sin(Math.PI*2*i/radarSkills.length - Math.PI/2)" r="6" fill="#fff" stroke="#52ddff" stroke-width="2.5"/>
          ...
          <div class="tech-section">
            ...
            <div class="skill-cards-grid">
              <div class="skill-card" v-for="s in radarSkills" :key="s.name" :style="{ '--accent': s.val >= 80 ? '#3ab6d4' : s.val >= 70 ? '#52ddff' : '#ffb65c' }">
                <div class="sc-header">
                  <span class="sc-name">{{ s.name }}</span>
                  <span class="sc-val" :style="{ color: s.val >= 80 ? '#3ab6d4' : s.val >= 70 ? '#52ddff' : '#ffb65c' }">{{ s.val }}</span>
                </div>
                <div class="sc-mini-ring">
                  <svg viewBox="0 0 60 60">
                    <circle cx="30" cy="30" r="24" fill="none" stroke="rgba(255,255,255,0.06)" stroke-width="5"/>
                    <circle cx="30" cy="30" r="24" fill="none" :stroke="s.val >= 80 ? '#3ab6d4' : s.val >= 70 ? '#52ddff' : '#ffb65c'" stroke-width="5" stroke-linecap="round" :stroke-dasharray="`${150.8 * s.val/100} 150.8`" transform="rotate(-90 30 30)"/>
                  </svg>
                </div>
        ...
        <div v-if="activePanel === 'jobs'" class="detail-page page-jobs">
          <div class="page-header">
            <div class="header-icon" style="background: linear-gradient(135deg, #52ddff20, #0aa9b420); border-color: #52ddff60;">
              <svg viewBox="0 0 24 24" fill="none" stroke="#52ddff" stroke-width="2"><rect x="2" y="7" width="20" height="14" rx="2"/><path d="M16 21V5a2 2 0 0 0-2-2h-4a2 2 0 0 0-2 2v16"/></svg>
            </div>
            <div class="header-titles">
              <h2>智能岗位推荐</h2>
              ...
              <div class="jr-info">
                ...
                <div class="jr-match-bar">
                  ...
                  <div class="jrmb-track">
                    <div class="jrmb-fill" :style="{ width: j.score + '%', background: j.score >= 70 ? 'linear-gradient(90deg, #3ab6d4, #52ddff)' : 'linear-gradient(90deg, #52ddff, #0aa9b4)' }"></div>
                  </div>
                  <div class="jrmb-val" :style="{ color: j.score >= 70 ? '#3ab6d4' : '#52ddff' }">{{ j.score }}%</div>
                </div>
              </div>
              <div class="jr-action">
        ...
        <div v-if="activePanel === 'ai-suggest'" class="detail-page page-ai">
          <div class="page-header">
            ...
            <div class="header-titles">
              <h2 style="background: linear-gradient(90deg, #c084fc, #52ddff); -webkit-background-clip: text; -webkit-text-fill-color: transparent;">AI 智能学习建议</h2>
        ...
        <div v-if="activePanel === 'weekly-plan'" class="detail-page page-plan">
          <div class="page-header">
            <div class="header-icon" style="background: linear-gradient(135deg, #3ab6d420, #52ddff20); border-color: #3ab6d460;">
              <svg viewBox="0 0 24 24" fill="none" stroke="#3ab6d4" stroke-width="2"><rect x="3" y="4" width="18" height="18" rx="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/></svg>
            </div>
            <div class="header-titles">
              <h2>本周成长计划</h2>
            ...
            <div class="progress-big">
              <svg viewBox="0 0 80 80">
                ...
                <circle cx="40" cy="40" r="32" fill="none" stroke="#3ab6d4" stroke-width="6" stroke-linecap="round" stroke-dasharray="120.6 201" transform="rotate(-90 40 40)" style="filter: drop-shadow(0 0 8px #3ab6d480)"/>
              </svg>
              <span><b>3</b>/5</span>
            </div>
          </div>
        ...
        <div v-if="activePanel === 'interview'" class="detail-page page-interview">
          <div class="page-header">
            <div class="header-icon" style="background: linear-gradient(135deg, #52ddff20, #0aa9b420); border-color: #52ddff60;">
              <svg viewBox="0 0 24 24" fill="none" stroke="#52ddff" stroke-width="2"><path d="M12 1a3 3 0 0 0-3 3v8a3 3 0 0 0 6 0V4a3 3 0 0 0-3-3z"/><path d="M19 10v2a7 7 0 0 1-14 0v-2"/><line x1="12" y1="19" x2="12" y2="23"/><line x1="8" y1="23" x2="16" y2="23"/></svg>
            </div>
            <div class="header-titles">
              <h2>AI 模拟面试</h2>
              <p>真实面试场景模拟 · 智能评估反馈</p>
              ...
              <svg viewBox="0 0 200 200">
                <defs>
                  <linearGradient id="intGrad" x1="0%" y1="0%" x2="100%" y2="100%">
                    <stop offset="0%" stop-color="#52ddff"/><stop offset="100%" stop-color="#a855f7"/>
                  </linearGradient>
                </defs>
                <circle cx="100" cy="100" r="85" fill="none" stroke="rgba(255,255,255,0.05)" stroke-width="12"/>
                <circle cx="100" cy="100" r="85" fill="none" stroke="url(#intGrad)" stroke-width="12" stroke-linecap="round" stroke-dasharray="416 534" transform="rotate(-90 100 100)" style="filter: drop-shadow(0 0 12px #52ddff80)"/>
              </svg>
          ...
          <div class="tech-section">
            ...
            <div class="interview-records">
              <div class="ir-item" v-for="(r, i) in interviewRecords" :key="i">
                ...
                <div class="ir-score" :style="{ color: r.score >= 75 ? '#3ab6d4' : r.score >= 65 ? '#52ddff' : '#ffb65c' }">{{ r.score }}分</div>
              </div>
            </div>
          </div>
        </div>
        ...
        <div v-if="activePanel === 'timeline'" class="detail-page page-timeline">
          <div class="page-header">
            <div class="header-icon" style="background: linear-gradient(135deg, #52ddff20, #a855f720); border-color: #52ddff60;">
              <svg viewBox="0 0 24 24" fill="none" stroke="#52ddff" stroke-width="2"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg>
            </div>
            <div class="header-titles">
              <h2>成长时间线</h2>
        ...
        <div v-if="activePanel === 'path'" class="detail-page page-path">
          <div class="page-header">
            <div class="header-icon" style="background: linear-gradient(135deg, #52ddff20, #a855f720); border-color: #52ddff60;">
              <svg viewBox="0 0 24 24" fill="none" stroke="#52ddff" stroke-width="2"><polyline points="4 17 10 11 4 5"/><line x1="12" y1="19" x2="20" y2="19"/></svg>
            </div>
            <div class="header-titles">
              <h2>成长路径</h2>
            ...
            <svg class="path-svg" viewBox="0 0 800 200">
              <defs>
                <linearGradient id="pathGrad" x1="0%" y1="0%" x2="100%" y2="0%">
                  <stop offset="0%" stop-color="#52ddff"/><stop offset="50%" stop-color="#a855f7"/><stop offset="100%" stop-color="#ec4899"/>
                </linearGradient>
              </defs>
              ...
              <path d="M 60 150 Q 150 50 260 120 T 400 110" fill="none" stroke="url(#pathGrad)" stroke-width="4" stroke-linecap="round" style="filter: drop-shadow(0 0 8px #52ddff80)"/>
            </svg>
          ...
          <div class="current-stage-card">
            <div class="csc-header">
              <span class="csc-badge" style="background: linear-gradient(135deg, #88ddff, #a855f7);">当前阶段</span>
              <h3>RAG 知识增强</h3>
            </div>
            ...
            <div class="csc-grid">
              ...
              <div class="csc-item"><span class="csc-i-label">提升匹配度</span><span class="csc-i-val" style="color: #3ab6d4;">+15%</span></div>
            </div>
          ...
          <div class="profile-hero">
            <div class="ph-avatar-ring">
              <svg viewBox="0 0 160 160">
                ...
                <circle cx="80" cy="80" r="72" fill="none" stroke="url(#avGrad)" stroke-width="4" stroke-dasharray="354 452" transform="rotate(-90 80 80)" style="filter: drop-shadow(0 0 10px #52ddff80)"/>
              </svg>
            ...
            <div class="ph-info">
              ...
              <div class="ph-tags">
                ...
                <span class="ph-tag" style="background: #3ab6d420; color: #3ab6d4;">学习中</span>
              </div>
...
const suggestions = computed(() => (matchDetail.value.ai_analysis?.suggestions || matchDetail.value.suggestions || []).filter(Boolean))
...
const activityEvents = computed(() => [
  ...
  ...interviews.value.map(item => ({ type: 'interview', id: item.id, date: item.completed_at || item.updated_at || item.created_at, title: item.status === 'completed' ? '完成模拟面试' : '开始模拟面试', detail: item.job_name || '岗位面试' })),
...
async function loadData() {
  loading.value = true
  try {
    const [p, r, m, i, tasks, resources] = await Promise.all([
      api.myProfile(), api.resumes(), api.matchAnalysisHistory(), api.interviewSessions(),
      api.learningTasks(), api.learningResources(),
    ])
    profile.value = p || {}
    resumes.value = r || []
    matches.value = m || []
...
async function toggleTask(task: any) {
  const saved = await api.updateLearningTask(task.id, task.status === 'completed' ? 'pending' : 'completed')
  learningTasks.value = learningTasks.value.map(item => item.id === saved.id ? saved : item)
}
...
const matchStats = [
  { label: '较上次提升', value: '14%', color: '#3ab6d4', icon: '↑', bar: 85 },
  { label: '超过同学', value: '72%', color: '#52ddff', icon: '◉', bar: 72 },
  { label: '技能缺口', value: '5项', color: '#ffb65c', icon: '!', bar: 40 },
  { label: '推荐课程', value: '8门', color: '#a855f7', icon: '★', bar: 60 },
]

const matchDims = [
  { name: '技术能力', val: 78, color: '#52ddff' },
  { name: '项目经验', val: 65, color: '#52ddff' },
  { name: '算法基础', val: 82, color: '#3ab6d4' },
...
const weakAreas = [
  ...
  { title: '大模型前沿', desc: 'Agent、多模态、RAG等新技术跟进较慢', suggest: '关注顶会论文，复现最新开源项目', icon: '🚀', color: '#52ddff' },
]
...
const resources = [
  { type: '文档', title: 'LangChain官方中文文档', source: 'python.langchain.com', rating: 4.8, color: '#3ab6d4' },
  { type: '视频', title: 'RAG从原理到实战完整版', source: 'B站-跟李沐学AI', rating: 4.9, color: '#52ddff' },
...
const weekStats = [
  { label: '学习时长', value: '12.5h', icon: '⏱', color: '#52ddff' },
  { label: '完成任务', value: '3/5', icon: '✓', color: '#3ab6d4' },
  { label: '练习题数', value: '28道', icon: '📝', color: '#a855f7' },
  { label: '连续学习', value: '5天', icon: '🔥', color: '#ffb65c' },
]

const weeklyTasks = ref([
  { text: '完成Transformer架构学习', date: '周一', cat: '理论', catColor: '#52ddff', points: 100, done: true },
  { text: '阅读《Attention Is All You Need》论文', date: '周二', cat: '论文', catColor: '#a855f7', points: 80, done: true },
  { text: '动手实现Multi-Head Attention机制', date: '周三', cat: '实践', catColor: '#3ab6d4', points: 150, done: true },
...
const interviewDims = [
  { name: '技术能力', val: 72, color: '#52ddff' },
  { name: '表达能力', val: 72, color: '#3ab6d4' },
...
const timelineEvents = [
  { date: '06-12', title: '深度学习基础课程结业', desc: '完成CNN/RNN/LSTM核心原理学习，通过结业考试（92分）', icon: '🎓', color: '#3ab6d4', reward: '深度学习入门徽章' },
  { date: '06-08', title: 'Python技能升级至Lv.18', desc: '累计完成328道编程练习题，进入熟练阶段', icon: '🐍', color: '#52ddff', reward: '代码达人' },
  ...
  { date: '05-10', title: '开启AI学习之旅', desc: '注册账号，完成入学测评，定制学习路径', icon: '🚀', color: '#0aa9b4', reward: '初出茅庐徽章' },
]

const pathNodes = [
  { name: 'Python', sub: '入门基础', done: true, current: false, color: '#52ddff', icon: 'Py' },
  { name: '机器学习', sub: '基础算法', done: true, current: false, color: '#52ddff', icon: 'ML' },
  { name: '深度学习', sub: '进阶应用', done: true, current: false, color: '#a855f7', icon: 'DL' },
  { name: 'RAG', sub: '知识增强', done: false, current: true, color: '#88ddff', icon: 'R' },
...
const profileStats = [
  { label: '学习天数', value: '68天', icon: '📅', color: '#52ddff' },
  { label: '完成课程', value: '24门', icon: '📚', color: '#3ab6d4' },
...
const badges = [
  ...
  { icon: '⭐', name: '学习达人', unlocked: true, color: '#52ddff' },
  ...
  { icon: '🚀', name: '项目大师', unlocked: false, color: '#3ab6d4' },
  { icon: '🎯', name: '目标达成', unlocked: true, color: '#52ddff' },
  { icon: '🌟', name: '知识新星', unlocked: true, color: '#88ddff' },
]
...
.page-header::after {
  ...
  background: linear-gradient(90deg, #52ddff, transparent);
}
.header-icon {
...
.st-line {
  ...
  background: linear-gradient(180deg, #52ddff, #8f7cff);
  border-radius: 2px;
...
.br-pct { font-size: 20px; color: #52ddff; font-weight: 600; margin-top: 2px; }
...
.gc-tags span {
  ...
  border-radius: 4px;
  font-size: 11px; color: #52ddff;
}
.gc-btn {
  padding: 7px 16px;
  background: linear-gradient(135deg, #52ddff, #0aa9b4);
...
.sc-level.lv-good { background: #3ab6d420; color: #3ab6d4; }
.sc-level.lv-mid { background: #52ddff20; color: #52ddff; }
...
.wc-suggest { font-size: 12px; color: #3ab6d4; margin-top: 6px; }
...
.ls-item.done .ls-connector { background: #3ab6d440; }
.ls-node {
...
.ls-item.done .ls-node { background: #3ab6d4; border-color: #3ab6d4; color: #050d1f; }
...
.progress-big span {
  position: absolute;
  font-size: 14px; font-weight: 700; color: #3ab6d4;
}
...
.task-item.done .ti-check { background: #3ab6d4; border-color: #3ab6d4; }
.ti-body { flex: 1; }
...
.ihsr-num { font-size: 52px; font-weight: 800; color: #52ddff; line-height: 1; text-shadow: 0 0 25px rgba(78,216,255,0.5); }
...
.ihsr-grade {
  ...
  border-radius: 10px;
  font-size: 13px; color: #3ab6d4; font-weight: 600;
}
.ih-info { flex: 1; }
...
.ph-avatar-inner {
  ...
  border-radius: 50%;
  background: linear-gradient(135deg, #52ddff, #a855f7);
  display: flex; align-items: center; justify-content: center;
...
.ph-tag {
  ...
  border-radius: 20px;
  font-size: 12px; color: #52ddff; font-weight: 500;
}
...
.phlb-fill { height: 100%; width: 78%; background: linear-gradient(90deg, #52ddff, #a855f7); border-radius: 4px; box-shadow: 0 0 10px rgba(78,216,255,0.4); }
.phlb-val { font-size: 14px; font-weight: 700; color: #52ddff; }
