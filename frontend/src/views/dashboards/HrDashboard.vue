    ...
    series: [
      {
        ...
        data: demand,
        itemStyle: {
          color: { type: 'linear', x: 1, y: 0, x2: 0, y2: 0, colorStops: [{ offset: 0, color: '#0aa9b4' }, { offset: 1, color: '#066a72' }] },
      ...
      },
      {
        ...
        data: supply,
        itemStyle: {
          color: { type: 'linear', x: 1, y: 0, x2: 0, y2: 0, colorStops: [{ offset: 0, color: '#0aa9b4' }, { offset: 1, color: '#042a30' }] },
          borderRadius: [0, 4, 4, 0]
        }
      }
    ]
...
const industryTrendOption = computed(() => {
  ...
  return {
    ...
    series: [{
      ...
      itemStyle: { color: '#52ddff', borderWidth: 2, borderColor: '#ffffff' },
  ...
  series: [
    {
      ...
      itemStyle: {
        color: { type: 'linear', x: 0, y: 0, x2: 0, y2: 1, colorStops: [{ offset: 0, color: '#52ddff' }, { offset: 1, color: '#066a72' }] },
  ...
  return {
    ...
    series: [{
      ...
      label: {
        ...
        rich: {
          title: { fontSize: 16, fontWeight: 'bold', color: '#0aa9b4', fontFamily: 'Orbitron' },
    ...
    tooltip: {
      ...
      formatter: (params: any) => {
        if (params.seriesType === 'effectScatter') {
          return `<b style="color:#52ddff">${params.name}</b><br/>人才：<b>${params.value?.[2] ?? 0}</b> 人`
        }
        if (params.data == null) return `${params.name}<br/><span style="color:#88a9c4">暂无数据</span>`
        return `${params.name}<br/><span style="color:#52ddff;font-weight:700">${params.value}</span> 人`
      }
    },
    visualMap: {
      type: 'continuous',
      ...
      inRange: { color: ['#042a30', '#053340', '#0aa9b4', '#52ddff', '#52ddff'] },
      calculable: false,
      itemWidth: 10,
      itemHeight: 70,
...
.cockpit-header__live::before { width: 22px; height: 2px; content: ""; background: #52ddff; }
@media (min-width: 900px) { .cockpit-header__live { display: inline-flex; } }
.live-dot { width: 6px; height: 6px; border-radius: 50%; background: #52ddff; box-shadow: 0 0 8px #52ddff; animation: ping 1.6s cubic-bezier(0,0,.2,1) infinite; }
...
.brain-icon { color: #52ddff; font-size: 22px; }
...
.cockpit-button .el-icon { color: #52ddff; }
...
.kpi-icon--cyan { background: rgba(54, 215, 255, .14); color: #52ddff; border: 1px solid rgba(54, 215, 255, .28); }
.kpi-icon--cyan-bright { background: rgba(54, 215, 255, .18); color: #b9e5ff; border: 1px solid rgba(54, 215, 255, .35); }
...
.kpi-value { color: #52ddff; font-size: 20px; font-weight: 800; margin-top: 4px; line-height: 1.1; }
...
.kpi-foot__delta.up { color: #52ddff; font-weight: 600; }
...
.panel-head h2::before, .panel-head h3::before { width: 3px; height: 16px; border-radius: 6px; content: ""; background: #52ddff; box-shadow: 0 0 9px rgba(54, 215, 255, 0.65); }
...
.legend-swatch--line { width: 8px; height: 2px; background: #52ddff; border-radius: 1px; }
...
.gap-circle--cyan .gap-circle__value { color: #52ddff; }
...
.action-row__icon--cyan { background: rgba(54, 215, 255, .12); color: #52ddff; border: 1px solid rgba(54, 215, 255, .28); }
...
.action-row__btn { padding: 5px 9px; font-size: 11px; color: #52ddff; background: rgba(8, 42, 92, .55); border: 1px solid rgba(54, 215, 255, .32); border-radius: 6px; cursor: pointer; white-space: nowrap; transition: background .2s, border-color .2s; }
...
.pano-kpi__delta.up   { color: #52ddff; border-color: rgba(34, 211, 238, 0.18); }
...
.text-link { background: transparent; border: 0; color: #52ddff; font-size: 11px; cursor: pointer; padding: 0; transition: color .2s; }
.text-link:hover { color: #b9e5ff; }
...
.region-top__row .font-digits { color: #52ddff; }
...
.info-modal__icon { width: 48px; height: 48px; border-radius: 50%; background: rgba(54, 215, 255, .14); border: 1px solid rgba(54, 215, 255, .4); display: flex; align-items: center; justify-content: center; margin: 0 auto; color: #52ddff; font-size: 20px; animation: bounce 1.4s infinite; }
