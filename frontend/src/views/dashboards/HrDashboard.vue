...
const gapOption = computed(() => ({
  ...
  series: [
    ...
    { name: '人才供给', type: 'bar', barWidth: 9, data: skillGap.value.slice(0, 8).map((item) => item.supply), itemStyle: { color: '#24d7b1', borderRadius: 5 }, animationDuration: 760 }
  ]
}))
const trendOption = computed(() => ({
  ...
  series: [{ type: 'line', smooth: .35, symbolSize: 6, data: (model.value.market?.software_revenue_trend || []).map((item: any) => item.value), lineStyle: { color: '#36c6e8', width: 2 }, itemStyle: { color: '#36c6e8' }, areaStyle: { color: 'rgba(54,198,232,.08)' }, animationDuration: 800 }]
}))
