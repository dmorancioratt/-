...
TASK_DEFINITIONS: dict[str, dict[str, Any]] = {
    ...
    "digital_interview": {
        "instruction": (
            ...
            "优先追问对方实际提到的项目、技术选择、困难、取舍和结果，禁止重复 asked_questions 中已经问过的问题。"
            "每轮只问一个清晰问题，围绕 input.next_focus 组织本轮考察，并避开 covered_focus 中已经覆盖充分的方向。"
            "面试目标为 input.target_scored_rounds 轮有效回答，remaining_scored_rounds 较少时问题要更聚焦。"
            "feedback 用一到两句话指出回答中具体做得好的地方和一个可改进点，不说空话；"
            "score_preview 必须按 0-95 分区间评分；胡乱回答、明显无效回答可以给 0 分，短答、无证据、无量化结果时不要给高分。"
...
def analyze_with_ai(task_type: str, payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if AI_PROVIDER not in {"deepseek", "chat_api"}:
        raise AIProviderError(f"不支持的 AI Provider：{AI_PROVIDER}")
    return analyze_with_chat_api(task_type, payload)
...
def analyze_with_mock(task_type: str, payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if task_type == "jd_parse":
        ...
        focus_questions = {
            ...
            "技术深度": "刚才的经历里哪个技术选择最关键？你为什么这样选，有没有替代方案？",
            "问题排查": "如果这个项目在线上出现异常，你会按什么顺序定位问题，并如何判断修复有效？",
            ...
            "关键难点": "项目推进中最难的一步是什么？约束、尝试过的方法和最终结果分别是什么？",
            "技术取舍": "你在这个项目里做过哪次重要技术取舍？请说明判断标准和代价。",
            "量化结果": "这个项目最后如何验收？有没有数据、用户反馈或业务指标能说明效果？",
        }
        next_focus = _string(payload.get("next_focus"))
        asked_questions = set(_string_list(payload.get("asked_questions")))

        def choose_focus_question() -> str:
            candidates = [focus_questions.get(next_focus), *fallback_questions]
            for candidate in candidates:
                if candidate and candidate not in asked_questions:
                    return candidate
            return fallback_questions[(round_number - 1) % len(fallback_questions)]
        ...
        if not answer or action == "start":
            ...
            question = choose_focus_question()
...
def analyze_with_chat_api(task_type: str, payload: dict[str, Any]) -> dict[str, Any]:
...
def _normalize_result(task_type: str, result: dict[str, Any], payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if task_type.startswith("rag_query_"):
        # 保留模型引用，调用方会逐字核对 quote 是否真实存在于检索片段。
        return {
