...
def start_session(uid: str = "") -> dict:
    if not _start_lock.acquire(blocking=False):
        raise VirtualHumanError("数字人正在建立连接，请稍候再试")
    try:
        return _start_session(uid)
    finally:
        _start_lock.release()


def _start_session(uid: str = "") -> dict:
    _require_config()
...
def _start_hls_transcoder(stream_url: str, handle: str) -> tuple[Path, subprocess.Popen]:
    ...
    while time.time() < deadline:
        ...
        if process.poll() is not None:
            ...
            if detail:
                logger.error("数字人视频转码启动失败: %s", detail[-1000:])
            raise VirtualHumanError(f"数字人视频转码启动失败：{detail[-500:] or f'FFmpeg 已退出（exit={process.returncode}）'}")
        time.sleep(0.25)
...
def _resolve_ffmpeg() -> str:
    configured = os.getenv("XUNFEI_FFMPEG_PATH", "").strip()
    if configured:
        return configured
    ...
    try:
        ...
        raise VirtualHumanError("本机缺少数字人视频转码组件，请安装 ffmpeg 或 imageio-ffmpeg") from exc
