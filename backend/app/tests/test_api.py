...
def test_evolution_derives_timeline_from_versioned_sources_without_events():
    with SessionLocal() as db:
        existing_events = db.scalars(select(EvolutionEvent)).all()
        snapshots = [
            {
                "id": event.id,
                "job_id": event.job_id,
                "added_skills": event.added_skills,
                "removed_skills": event.removed_skills,
                "modified_skills": event.modified_skills,
                "update_note": event.update_note,
                "data_sources": event.data_sources,
                "confidence": event.confidence,
                "version_record": event.version_record,
                "evidence": event.evidence,
                "created_at": event.created_at,
            }
            for event in existing_events
        ]
        db.execute(delete(EvolutionEvent))
        db.commit()

    try:
        timeline = client.get("/api/evolution/timeline")
        assert timeline.status_code == 200
        payload = timeline.json()
        assert payload["derived"] is True
        assert payload["total"] >= 3
        assert payload["timeline"]
        assert payload["events"][0]["added"]
        assert payload["events"][0]["dataSources"]

        cards = client.get("/api/evolution/version-compare")
        assert cards.status_code == 200
        card_payload = cards.json()
        assert card_payload["total"] >= 3
        assert card_payload["cards"][0]["derived"] is True
        assert card_payload["cards"][0]["added"]
    finally:
        with SessionLocal() as db:
            db.execute(delete(EvolutionEvent))
            for snapshot in snapshots:
                db.add(EvolutionEvent(**snapshot))
            db.commit()
...
def test_interview_auto_completes_with_bounded_scores_and_advice():
    login = client.post("/api/auth/login", json={"username": "student_demo", "password": "Demo@123"})
    headers = {"Authorization": f"Bearer {login.json()['token']}"}
    session_id = None
    try:
        opening = client.post(
            ...
            json={
                "job_name": "AI 应用开发工程师",
                "resume_summary": "做过 Python 数据分析项目",
                "interview_style": "adaptive",
                "action": "start",
            },
        )
        assert opening.status_code == 200
        ...
        first_scores = short_answer.json()["history"][0]["score_preview"]
        assert set(first_scores.values()) == {0.0}
        ...
        for index in range(2, 7):
            response = client.post(
                ...
                json={
                    "job_name": "AI 应用开发工程师",
                    "interview_session_id": session_id,
                    "candidate_answer": f"第 {index} 轮我负责 Python 项目，实现数据处理并让指标提升 {index * 10}%。",
                    "action": "answer",
                },
            )
            assert response.status_code == 200
        ...
        assert payload["interview_session"]["status"] == "completed"
        assert payload["interview_session"]["round_count"] == 6
        assert len(payload["history"]) == 6
        assert payload["final_report"]["rounds_scored"] == 6
        assert payload["final_report"]["target_rounds"] == 6
        assert payload["final_report"]["hiring_recommendation"]
        assert payload["final_report"]["next_steps"]
