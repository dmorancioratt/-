...
def run_core() -> list[EvalResult]:
    """Fast evaluation set used by online dashboard endpoints.

    Keep heavier RAG retrieval evaluation in ``run()`` for offline command-line
    reproduction so page loads do not depend on embedding model availability.
    """
    from app.evaluation.generate_match_predictions import generate
